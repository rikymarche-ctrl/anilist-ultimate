...
import { AstraDomService } from './services/AstraDomService';
...
export class AstraModule extends BaseModule {
  ...
  constructor(
    ...
    @inject(TOKENS.AstraDomService) private domService: AstraDomService,
  ...
  public async init(): Promise<void> {
    ...
    this.onPageChange(async (event) => {
      ...
      this.domService.enhanceCards(path);
    });
    ...
    this.sharedObserver.register('astra-enhancer', () => {
      ...
      this.domService.enhanceBrowseDropdown();
    });
    ...
    this.domService.enhanceCards(currentPath);
    this.domService.injectNavbarButton(() => this.eventBus.emit(EVENT_TYPES.ASTRA_OPEN));
    this.domService.hijackMediaButton((id) => this.ratingModal.open(id));
